...
export const AuthPage: React.FC = () => {
  ...
  const handleSignInSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        dispatch(loginUser({
          email: user.email,
          pharmacistName: isEmergencyDesk ? 'Dr. S. Reddy (Chief Emergency)' : user.pharmacistName,
      ...
      setLoginError(`${errMsg} (Attempt ${attempts})`);
    }
  };
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSignUpError(error.response?.data?.message || 'Failed to register account.');
    }
  };
  ...
  const handleSendResetLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTargetAccountName(email);
      // NOTE: token is usually sent via email. For local testing, we might need a way to grab it, 
      // but assuming the user will check logs or email.
      setResetToken('check-backend-logs'); 
      setTimeout(() => {
  ...
  const handleResetPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/auth/reset-password', {
        token: resetToken, // User would normally get this from URL
        newPassword: newPass
      });

      setSignUpNotice(`Password updated successfully! Please sign in with your new password.`);
