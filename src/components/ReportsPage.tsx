...
export const ReportsPage: React.FC = () => {
  ...
  const handleExportCSV = () => {
    ...
    if (activeTab === 'GST') {
      ...
      MOCK_HSN_TAX_REPORT.forEach(row => {
      ...
      MOCK_TOP_MEDICINES.forEach(row => {
        csvContent += `"${row.name}","${row.salt}",${row.qtySold},${row.revenue},${row.margin}%\n`;
      });
    } else {
      csvContent += 'Metric,Value\n';
      csvContent += `Gross Sales,INR ${MOCK_SALES_SUMMARY.grossSales}\n`;
      csvContent += `Discounts,INR ${MOCK_SALES_SUMMARY.discountsGiven}\n`;
      csvContent += `Total CGST,INR ${MOCK_SALES_SUMMARY.totalCGST}\n`;
      csvContent += `Total SGST,INR ${MOCK_SALES_SUMMARY.totalSGST}\n`;
      csvContent += `Net Revenue,INR ${MOCK_SALES_SUMMARY.netRevenue}\n`;
      csvContent += `Total Bills,${MOCK_SALES_SUMMARY.totalBills}\n`;
    }
    ...
    <div className="flex-1 overflow-y-auto bg-slate-100 p-4 space-y-4 font-sans select-none">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        ...
        >
          ...
          <p className="text-base font-black text-slate-900 font-heading">₹{MOCK_SALES_SUMMARY.netRevenue.toLocaleString('en-IN')}</p>
        ...
        >
          ...
          <p className="text-base font-black text-slate-900 font-heading">₹{MOCK_SALES_SUMMARY.totalTax.toLocaleString('en-IN')}</p>
        ...
        >
          ...
          <p className="text-base font-black text-slate-900 font-heading">{MOCK_TOP_MEDICINES[0].name.split(' ')[0]}</p>
        ...
        >
          ...
          <p className="text-base font-black text-slate-900 font-heading">{MOCK_PAYMENT_SPLIT[0].percent}% UPI</p>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs">
              ...
              <p className="text-xl font-black text-slate-900 font-heading mt-0.5">₹{MOCK_SALES_SUMMARY.grossSales.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Discounts Allowed</p>
              <p className="text-xl font-black text-emerald-700 font-heading mt-0.5">-₹{MOCK_SALES_SUMMARY.discountsGiven.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Total Bills</p>
              <p className="text-xl font-black text-slate-900 font-heading mt-0.5">{MOCK_SALES_SUMMARY.totalBills}</p>
            </div>
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-xs">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Avg Bill Value</p>
              <p className="text-xl font-black text-slate-900 font-heading mt-0.5">₹{MOCK_SALES_SUMMARY.avgBillValue}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden space-y-3">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center flex-wrap gap-2">
            ...
            <div className="flex space-x-3 text-xs font-bold">
              <span className="text-slate-700">Total CGST: <strong className="text-emerald-800">₹{MOCK_SALES_SUMMARY.totalCGST}</strong></span>
              <span className="text-slate-700">Total SGST: <strong className="text-emerald-800">₹{MOCK_SALES_SUMMARY.totalSGST}</strong></span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs" style={{ minWidth: '650px' }}>
              ...
              <tbody className="divide-y divide-slate-100">
                {MOCK_TOP_MEDICINES.map((med, idx) => (
                  <tr key={med.name} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-3 py-2.5 text-slate-600">{med.salt}</td>
                    <td className="px-3 py-2.5 text-center font-bold text-slate-900">{med.qtySold}</td>
                    <td className="px-3 py-2.5 text-right font-black text-emerald-800">₹{med.revenue.toFixed(2)}</td>
                    <td className="px-4 py-2.5 text-right font-bold text-slate-700">{med.margin}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
